...
+ 路由器管理密碼:
  ...
  ```
+ 嘗試使用LAN或WIFI取得利用DHCP取得IP。依照分配IP找到管理頁面網址，並使用路由器管理帳號密碼嘗試登入。若登入成功即可從網頁確認韌體版本。  
+ 如果拿到的設備不是出廠狀態無法登入管理頁面，請使用RESET按鈕恢復原廠設定。常見的恢復方法如下:  
  ...
  + 出廠預設的RESET按鈕的規定如下:
    ...
    + 超過20秒=沒有事情發生
    ```    
    基本上利用清除使用者設定就夠讓你用機身上的帳號密碼登入路由器了。  
    此外，為了避免日後忘記，不建議修改RESET按鈕的規定，若有修改請務必說明清楚。  
    ```
  + 具體操作方式(長按6-11秒):
...
+ 注意事項:
  ...
  + 更新到新韌體，我建議恢復原廠設定後再重新設定各項內容。恢復原廠設定方法如下:
    + 
### 快速安裝
