...
+ 用管理員帳號cgrg.service建立一個新使用者MTU5C
 ...
 + 產生金鑰(ubuntu 18):
   ...
   + 產生:
     + ssh-keygen -b 4096 -C "Key for secure MTU-5C file transfer"
 ...
 + 產生金鑰WIN:
   ...
   + 修改編碼為Linux適用:
     + 用notepad++打開剛剛的三個檔案，看右下角如果顯示**Windows(CR LF)**，用滑鼠雙擊選到**Unix(LF)**；如果編碼**不是**顯示**UTF-8**，用滑鼠雙擊選到**轉換成UTF8**，然後存檔。
  + 測試使用金鑰登入SFTP
    + 用「FileZilla-3.66.5」
    + 在站台管理員選SFTP，設定主機IP，PORT留白，登入形式選「一般」，會可以輸入帳號密碼，就是上面的MTU5C // 45002931。
